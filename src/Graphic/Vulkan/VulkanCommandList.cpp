...
void VulkanCommandList::bindMaterial(const Material* material)
{
	AKA_ASSERT(m_recording, "Trying to record something but not recording");
	...
	if (material->program->bindings.count > 0)
	{
		...
		vkCmdBindDescriptorSets(vk_command, bindPoint, vk_program->vk_pipelineLayout, 0, 1, &vk_material->vk_descriptorSet, 0, nullptr);
	}
}
