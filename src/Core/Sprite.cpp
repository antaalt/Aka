...
Sprite::Frame getFrame(const Aseprite& ase, const Aseprite::Frame& aseFrame)
{
...
Sprite Sprite::parse(const Path& path)
{
	...
	if (ase.tags.size() == 0)
	{
		...
		for (Aseprite::Frame& aseFrame : ase.frames)
			animation.frames.push_back(getFrame(ase, aseFrame));
	...
	else
	{
		for (Aseprite::Tag& aseTag : ase.tags)
		{
			...
			for (Aseprite::Word iFrame = aseTag.from; iFrame <= aseTag.to; iFrame++)
				animation.frames.push_back(getFrame(ase, ase.frames[iFrame]));
			sprite.animations.push_back(animation);
		}
	}
	return sprite;
