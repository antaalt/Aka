...
struct Mesh
{
	...
	gfx::BufferHandle vertices[8]; // Vertices buffers. Maximum of 1 / binding.
	gfx::BufferHandle indices; // Indices buffer

	gfx::IndexFormat format; // Index format if indexed.
	uint32_t count; // Vertices / Indices count depending on isIndexed

	bool isIndexed() const { return indices.data != nullptr; }
